#!/bin/bash
while true; do
	if [ -f notifications.txt ]; then
	    while IFS='' read -r line || [[ -n "$line" ]]; do
			QUESTION="$line"
			echo "${QUESTION}"
			# Auth token (replace with yours).
			TOKEN=`cat //pushNotifications/token.dat`
			...
			echo -e $POST_DATA_START > /home//pushNotifications/multipart_body.txt
			...
			rm /home//pushNotifications/tmp/pipe.wav
			ln -s /dev/stdout /home//pushNotifications/tmp/pipe.wav
			pico2wave -w /home//pushNotifications/tmp/pipe.wav "${QUESTION}" | tee /home//pushNotifications/pico2wav.wav | sox - -c 1 -r 16000 -e signed -b 16 -t wav - >> /home/karanvasnani/Desktop/AlzCare/pushNotifications/multipart_body.txt

			# Then we append closing boundary to request body file.
			echo -e $POST_DATA_END >> /home//pushNotifications/multipart_body.txt
			...
			curl -s -X POST \
			  ...
			  | tee /home//pushNotifications/response.mp3 | play -t mp3 -q -
		done < "notifications.txt"
